...
const workers = [
  { key: 'reed', persona: 'reliable', fullName: 'Reed Dalton Reliable 162225', username: 'reed-reliable-162225' },
  { key: 'lane', persona: 'forgetful', fullName: 'Lane Morris Forgetful 170111', username: 'lane-forgetful-170111' },
  { key: 'ty', persona: 'bad_at_tech', fullName: 'Ty Brooks Badtech 170111', username: 'ty-badtech-170111' },
  { key: 'milo', persona: 'general', fullName: 'Milo Test Worker 783163', username: 'milo-worker-783163' },
];
...
async function collectVisibility(page, worker) {
  ...
  const routes = ['/app', '/app/today', '/app/work-orders', '/app/time', '/app/team', '/app/payroll', '/app/settings', '/app/herd', '/app/land', '/app/needs-attention', '/admin', '/checkout'];
...
async function generalWorkerPermissionAndMistakeAttempts(page, worker) {
  ...
  const restrictedRoutes = ['/app/team', '/app/payroll', '/app/settings', '/app/herd', '/app/land', '/app/needs-attention', '/admin', '/checkout'];
...
(async () => {
  // Desktop browser for reed/lane/milo (fresh context per worker)
  const browser = await chromium.launch({ headless: true });
  try {
    for (const key of ['reed', 'lane', 'milo']) {
  ...
  // Ty - bad-at-tech mobile
  {
    const ty = workers.find((w) => w.key === 'ty');
    if (ty) {
      await badAtTechMobileFlow(ty);
    }
  }
